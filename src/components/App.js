...
const App = () => {
  ...
  const City = ({ name, index }) => {
    return <li key={`location${index + 1}`}>{name}</li>;
  };

  return (
    <div id="main">
      {/* Do not remove the main div */}
      <h1>Indian Cities for Domestic Tourism</h1>
      <ol>
        {indianCities.map((city, index) => (
          <City key={`location${index +1}`} name={city.name} index={index} />
        ))}
      </ol>
    </div>
  );
